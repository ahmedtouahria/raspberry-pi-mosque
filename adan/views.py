        ...
        if self_mosque:
            ...
            try:
                ...
                elfajer_time = datetime.strptime(
                    prayer_json_without_pm["elfajer"], "%H:%M:%S")
                duhr_time = datetime.strptime(
                    prayer_json_without_pm["duhr"], "%H:%M:%S")
                alasr_time = datetime.strptime(
                    prayer_json_without_pm["alasr"], "%H:%M:%S")
                almghreb_time = datetime.strptime(
                    prayer_json_without_pm["almaghreb"], "%H:%M:%S")
                alaicha_time = datetime.strptime(
                    prayer_json_without_pm["alaicha"], "%H:%M:%S")
                from .utils import get_time_near_prayer, get_total_seconds
                time_now_seconds = current_time.hour*3600 + \
                ...
                prayer_detect_time = datetime.strptime(
...
class TurnOnOffSpeaker(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            self_mosque = Mosque.objects.get(topic__user=request.user)
        except Mosque.DoesNotExist:self_mosque=None
        if command is not None:
            ...
            if self_mosque is not None:
                mqtt_publisher.main(topic=f"raspberry_pi/{self_mosque.topic.serial_number}", message=json_msg_publisher)
            else: return Response({"success":False,"msg":"this mosque does not have a topic"},status=status.HTTP_400_BAD_REQUEST)
            return Response({"success": True, "data": {"command": command}}, status=status.HTTP_200_OK)
        else:
            return Response({"success": False, "message": "command must be not null"}, status=status.HTTP_400_BAD_REQUEST)
